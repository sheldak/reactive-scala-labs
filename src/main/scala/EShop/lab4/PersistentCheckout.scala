...
class PersistentCheckout {
  ...
  def eventHandler(context: ActorContext[Command]): (State, Event) => State = (state, event) => {
    ...
    // event match {
    //   case CheckoutStarted        => SelectingDelivery
    //   case DeliveryMethodSelected => SelectingPaymentMethod
    //   case PaymentStarted         => ProcessingPayment
    //   case CheckoutClosed         => Closed
    //   case CheckoutCancelled      => Closed
    // }
  }
}
